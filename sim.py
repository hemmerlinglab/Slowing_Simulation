...
for k in range(tsteps-1):
    ...
    for n in part_arr[do_simu]:
        ...
        B0 = hbar * k_vec * v_max / (muB * gF)
        if (sx[n, k] > L_start) and (sx[n, k] < L_start + Zeeman_length):
            detuning = k_vec * (vx[n, k] - laser_detuning) - muB * gF * B0/hbar * np.sqrt(1 - (sx[n, k] - L_start)**2/Zeeman_length**2)
        else:
            detuning = k_vec * (vx[n, k] - laser_detuning)
        ...
        # if particle hit the wall or if particle turns around, stop simulation
...
arr = save_to_arr(['no_of_particles', 'dt','laser_detuning','slowing_time','ts','sx','vx','sx_noslow','vx_noslow'])
